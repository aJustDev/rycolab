...
| `ticks` | guard interval: margins read back, WHEA, CPU load, package W, state, AC line, battery W / % / Wh, EC temperatures and fans, power and GPU mode, panel Hz and brightness; since 0.4.0 the hottest core and its temperature, mean core V and highest core GHz (PM table), seconds since the last user input, charging W and charge mode, whether the dGPU is on the bus, the Windows overlay, the SMU read latency in ms, and with the dGPU on the bus its clock, memory clock, W, temperature, utilisation, whether the GPU curve profile is on and the driver resets seen (`docs/gpu.md`) | Guard |
